...
class MenuParser:
    ...
    async def parse_all(self, max_concurrent: int = 10, restaurant_ids: Optional[List[str]] = None):
        """
        Parse menus for all restaurants, optionally filtered by restaurant IDs.
        
        Args:
            max_concurrent: Maximum concurrent requests
            restaurant_ids: Optional list of restaurant IDs to filter by. If None, processes all restaurants.
        """
        # Load restaurants
        ...
        if restaurant_ids:
            ...
            restaurants = [
                ...
                if (r.get('id') in restaurant_ids_set or 
                    r.get('place_id') in restaurant_ids_set)
            ]
            if not restaurants:
                print(f"No restaurants found matching the provided IDs: {restaurant_ids}")
                return
            print(f"Filtered to {len(restaurants)} restaurant(s) matching provided IDs")
...
async def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help='Comma-separated list of restaurant IDs to process (e.g., "id1,id2,id3"). If not provided, processes all restaurants.'
    )
    ...
    restaurant_ids = None
    if args.ids:
        restaurant_ids = [id.strip() for id in args.ids.split(',') if id.strip()]
        print(f"Processing {len(restaurant_ids)} specific restaurant(s)")
    
    parser_instance = MenuParser(args.input, args.output)
    await parser_instance.parse_all(max_concurrent=args.max_concurrent, restaurant_ids=restaurant_ids)
